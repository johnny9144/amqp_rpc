...
class RPC {
  ...
  async callRPC ( qName, data, timeout = 30) {
    ...
    try {
      ...
      return new Promise( async ( resolve, reject) => {
        ...
        ch.sendToQueue( qName, Buffer.from( _.isString( data) ? data : JSON.stringify( data)), { correlationId: uuid(), replyTo: qok.queue });
      })
  ...
  async callPublish ( exName, qName, data) {
    try {
      ...
      this.ch.publish( exName, qName, Buffer.from( _.isString( data) ? data: JSON.stringify( data)));
    } catch ( e) {
